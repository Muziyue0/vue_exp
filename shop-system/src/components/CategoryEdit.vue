...
import { ElNotification } from 'element-plus' // 确保引入 ElNotification
...
const loadCategory = async () => {
    ...
    showMore.value = form.pid != 0 || form.id === 0 // 新增分类时 showMore 设置为 true
}
...
const resetForm = id => {
    form.id = id
    form.pid = '' // 清除 pid
    form.picture = '' // 清除 picture
    fileList.value = [] // 清除 fileList
    if (id === 0) {
        showMore.value = false // 新增分类时 showMore 设置为 false
    } else {
        showMore.value = form.pid != 0 || form.id === 0 // 修改分类时 showMore 设置为 true
    }
    btnCancel()
}
...
const uploadSuccess = response => {
    ...
    if (errno !== 0) {
        ...
        const fileName = data.savepath.substring(data.savepath.lastIndexOf('/') + 1)
        if (fileName) {
            fileList.value = [{ name: fileName, url: data.savepath }]
        }
    }
}
</script>
